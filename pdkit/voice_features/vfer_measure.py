...
def compute_vfer_measure(data, fs, VF_close, VF_open):
    ...
    for f1, f2 in zip(Fc1, Fc2):
        ...
        taps = scipy.signal.firwin(numtaps, [low, high], window='hamming', pass_zero=False)
        filters.append(taps)
    ...
    for seg_idx in range(len(VF_close)-1):
        tseries = data[VF_close[seg_idx]:VF_close[seg_idx+1]]
        ...
        if len(tseries) > 50:
            ...
            for ii, taps in enumerate(filters):
                # Use lfilter for FIR
                thanasis = scipy.signal.lfilter(taps, 1.0, segment_sig)
            ...
            for i in range(Hilb_env.shape[1]):
                for j in range(Hilb_env.shape[1]):
                    corr = np.correlate(Hilb_env[:, i], Hilb_env[:, j], mode='full')
                    max_corr = max(max_corr, np.max(corr))
            NEm.append(max_corr)
    ...
    VFlog_SEO = np.mean(log_bb(signal_BW_SEO), axis=0)
    
    with np.errstate(divide='ignore', invalid='ignore'):
        signal_BW_TKEO2 = np.mean(np.log(signal_BW_TKEO), axis=0)
        signal_BW_TKEO2 = np.nan_to_num(signal_BW_TKEO2, neginf=-100.0)
    ...
    N = len(VFTKEO)
    VFER[3] = np.sum(VFTKEO[:min(5, N)]) / np.sum(VFTKEO[min(5, N):min(10, N)])
    VFER[4] = np.sum(VFSEO[:min(5, N)]) / np.sum(VFSEO[min(5, N):min(10, N)]) 
    VFER[5] = np.sum(signal_BW_TKEO2[min(5, N):min(10, N)]) / np.sum(signal_BW_TKEO2[:min(5, N)])
    VFER[6] = np.sum(VFlog_SEO[min(5, N):min(10, N)]) / np.sum(VFlog_SEO[:min(5, N)])
